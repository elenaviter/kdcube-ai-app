...
def _extract_enhanced_backtrack(result, kb_instance, query: str) -> BacktrackInfo:
    """Extract enhanced backtrack information from search result."""
    try:
        # Get resource and version info
        search_metadata = getattr(result, 'search_metadata', {})
        resource_id = search_metadata.get('resource_id', '')
        version = search_metadata.get('version', '1')

        # Build RNs
        raw_rn = f"ef:{kb_instance.project}:knowledge_base:raw:{resource_id}:{version}"
        extraction_rn = f"ef:{kb_instance.project}:knowledge_base:extraction:{resource_id}:{version}:extraction_0.md"
        segmentation_rn = f"ef:{kb_instance.project}:knowledge_base:segmentation:retrieval:{resource_id}:{version}:segments.json"

        # Get navigation info from the result if available
        navigation = []
        if hasattr(result, 'segment') and result.segment:
            metadata = result.segment.get('metadata', {})
            base_segment_guids = metadata.get('base_segment_guids', [])

            if base_segment_guids:
                # Get base segments for navigation
                segmentation_module = kb_instance.get_segmentation_module()
                ...
                for guid in base_segment_guids:
                    if guid in base_lookup:
                        ...
                        # Check if query is in this segment
                        citations = [query] if query.lower() in base_seg.text.lower() else []
        ...
        logger.error(f"Error extracting backtrack info: {e}")
        return BacktrackInfo(
            raw={"citations": [query], "rn": ""},
            extraction={"related_rns": [], "rn": ""},
            segmentation={"rn": "", "navigation": []}
        )
