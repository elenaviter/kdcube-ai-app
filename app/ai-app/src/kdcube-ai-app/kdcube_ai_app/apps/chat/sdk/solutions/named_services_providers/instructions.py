...
CONTRACT FIRST. A namespace's schema is your working map of it: object kinds, search semantics, account targeting, and exact operation payloads. Projection-enabled providers disclose it progressively. `object_schema(namespace=...)` returns the root catalog. For a large realm, search its CAPABILITIES with `object_schema(namespace=..., query=...)` or browse one returned `schema_path`; this does not search the realm's user objects. Then pass the returned `object_kind` and `schema_operation` for the exact executable contract. A concrete `object_ref` can select its kind directly. Before `search_objects`, `list_objects`, `get_object`, `object_action`, `upsert_object`, `delete_object`, or `host_file`, have either that exact operation view or an explicit full schema visible in your timeline. A provider without projections may return its full schema directly. A consent or authority denial from `object_schema` means its contract remains unavailable: resolve the surfaced consent, retry `object_schema`, and construct the later call from the successful schema result. The current structured consent demand is authoritative: relay its `operation` and `claims` exactly. `claims: []` means only the operation boundary is missing; never infer or add claims from an earlier denial, the operation schema, or memory. Plan each call only from the visible contract; when it cannot express the user's request, say so and ask.
...
def named_services_bridge_instructions(
    *,
    ...
    steps.append(
        ...
        f"without projections may return its full schema directly. A consent or authority denial from `{schema}` "
        f"means its contract remains unavailable: resolve the surfaced consent, retry `{schema}`, and construct "
        f"the later call from the successful schema result. Plan each call from what `{schema}` names; "
    ...
    steps.append(
        ...
        "consent. The current structured consent demand is authoritative: explain it to the user and relay its "
        "`operation` and `claims` exactly, "
        "plus any reason and connection link the error carries. `claims: []` means only the operation boundary is "
        "missing; never infer or add claims from an earlier denial, the operation schema, or memory. When the "
