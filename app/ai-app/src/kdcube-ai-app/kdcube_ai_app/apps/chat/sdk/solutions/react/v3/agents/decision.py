...
def build_decision_system_text(
    *,
    ...
    # The channel contract teaches the BASE channels (thinking, action,
    # optional summary). A connected tool may extend the protocol with a
    # channel of its own — exec adds <channel:code> — and that channel's
    # rules render ONLY when the tool is in the effective roster: the
    # catalog is the authority on capability, and teaching a channel whose
    # tool is absent makes the agent believe it can execute code.
    protocol_tool_ids = {
    ...
    if _multi_action_enabled(multi_action_mode):
        protocol = (
            ...
            + (
                ...
                "The block above is the strategy. This protocol's technique is channels: <channel:thinking>, <channel:action>, optional <channel:summary>. Each action you emit lives in its own <channel:action> instance. When the strategy permits multi-action, repeat <channel:action>; when not, emit one <channel:action> and stop.\n"
            )
            + channel_extension_note +
            ...
            + (
                ...
                if exec_present else
                "CRITICAL: you have 3 channel types. Two are required every round; summary is allowed ONLY on complete/exit final-answer rounds.\n"
                "Output protocol (strict): one round = at least one <channel:thinking> and one or more <channel:action> (multiple only when the multi-action gate above passes).\n"
                "<channel:thinking> ... </channel:thinking>\n"
                "<channel:action> ... </channel:action>\n"
            )
            ...
            + react_summary_channel_details(memsearch_bound=(MEMSEARCH_TOOL_ID in protocol_tool_ids), multi_action=True) + "\n"
            "\n"
        ...
        protocol = (
            ...
            + (
                ...
                "DO NOT DO THIS: emit a sequence of channel groups (<channel:thinking></channel:thinking><channel:action></channel:action>) twice in the same response — this protocol allows exactly one of each per response.\n"
            )
            +
            ...
            + react_summary_channel_details(memsearch_bound=(MEMSEARCH_TOOL_ID in protocol_tool_ids)) + "\n"
            "\n"
            "[CHANNEL CITATION] (CRITICAL — streaming infra sensitive)\n"
            + (
                "Whenever you REFER to a channel BY NAME inside prose — in `notes`, in `thinking`, in `final_answer`, in a tool param, in code comments, or anywhere that is NOT the actual channel boundary — you MUST write it in BACKTICKS, e.g. `channel:thinking`, `channel:action`, `channel:code`, `channel:summary`.\n"
