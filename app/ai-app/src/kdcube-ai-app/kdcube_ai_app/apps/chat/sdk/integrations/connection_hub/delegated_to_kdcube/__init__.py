...
"""KDCube storage/runtime bindings plus Connection Hub delegation contracts."""
...
_CORE = "connection_hub.delegated_to_kdcube"
_HOST = __name__

# Package import has always registered the built-in adapters. Preserve that
# contract while the provider implementations are owned by Connection Hub.
import_module(f"{_CORE}.providers")
...
for name in (
    ...
    _EXPORTS[name] = f"{_CORE}.adapters"
for name in ("DelegatedToKdcubeBroker", "broker_for_user"):
    _EXPORTS[name] = f"{_HOST}.broker"
_EXPORTS["DelegatedToKdcubeClient"] = f"{_HOST}.client"
for name in (
...
for name in (
    ...
    _EXPORTS[name] = f"{_CORE}.oauth"
for name in ("DelegatedToKdcubeOperations", "operations_for_user"):
    _EXPORTS[name] = f"{_HOST}.operations"
for name in (
    "CONSENT_NEEDED_CODE",
    "PREFLIGHT_SCHEMA",
