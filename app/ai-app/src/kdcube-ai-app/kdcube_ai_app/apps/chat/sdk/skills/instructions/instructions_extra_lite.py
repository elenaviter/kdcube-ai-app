...
- Large/capped data: `[... PREVIEW TRUNCATED]`, `omitted`, `capped`, `[1-40]/180` markers = incomplete. Procedure: `stats_only` → `react.rg` if searchable → ranged `react.read` per affected region → act only when the needed regions are visible. Tool output is capped and is NOT a read channel. Full text needed and within caps → `max_text_symbols >= text_symbols`, verify not truncated. Oversized image → `react.read` its `conv:fi:` path (bounded downscaled view, `image_view`). Oversized PDF/binary → a file-processing tool extracts/splits into smaller derived artifacts, then read those. Never claim you inspected all content from a capped preview; name the recovery method used.
