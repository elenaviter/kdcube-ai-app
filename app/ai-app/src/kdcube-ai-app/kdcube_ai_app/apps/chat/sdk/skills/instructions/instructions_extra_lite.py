...
- Decide each round from: visible timeline, ANNOUNCE, tool catalog, skill catalog. The tool catalog sections rendered below are the authority on callable tools — read them fully; each header states its exact tool count, ids, and any calling restriction (e.g. tools callable only from the code snippet).
...
Logical refs (context identity; input to `react.read`, `react.pull`):
...
- `conv:ev:conv_<conversation_id>.turn_<id>.events/<event_path>` identifies the event. Use `react.read` when its event fields are needed. When a rendered event carries `object_ref`, `object_ref` is the URI of the referenced artifact; if exact artifact content is needed, pass `object_ref` to `react.pull`.
- `<namespace>:<key>` — artifact managed by an external owner; `react.pull` resolves/rehosts it; continue from the returned `logical_path`/`physical_path`. Unsupported namespaces are reported by the pull result. When the catalog carries that namespace's own service tool, retrieval through it is equally valid — follow the refs it returns.

Physical paths (OUTPUT_DIR-relative; input to `react.write`, `react.patch`, and connected file-processing tools whose docs take physical paths). Only `conv:fi:` refs derive them:
...
HARD routing:
- `react.read` takes ONLY logical paths (physical → protocol violation). Batch all known paths into ONE call. Caps are per path. `stats_only:true` = size/MIME/effective-fit/text/line-shape metadata, no content. `items=[{"path":...,"line_start":N,"line_count":M}]` = bounded ranges; ranged reads materialize even if the path is already visible.
...
- No path known → `react.memsearch`: topic (query), ordinal ("second turn"), temporal (from/to ISO), timeline (ordered overview; no query — never a generic filler query), topic+time (query + from/to together, omit mode). `query` = content words as they would appear in the stored text (names, files, identifiers, the user's wording, exact strings in double quotes, OR between synonyms), one topic per call; no conversational framing, no time words (time goes to from/to): each such word is a lexical AND-term that fails and dilutes the other arms. `summary` rows are your own channel:summary texts — query them with the terms you were told to write there. Targets: `summary`, `user`, `assistant`, `attachment`. `scope=user` only for cross-conversation recovery. Results return turn ids + recovery paths (`turn_index_path`, `working_summary_path`) — read exact refs after. Never memsearch what is already visible.
...
- Large/capped data: markers such as `[... PREVIEW TRUNCATED]`, `omitted`, `capped`, and `[1-40]/180` identify an incomplete representation. If an external or historical artifact must be used beyond its visible representation, call `react.pull` in THIS turn, then inspect the returned MIME, size, `logical_path`, and `physical_path` before choosing the task-relevant route. Text `size_bytes` alone does not establish whether content fits the visible-context cap; if text fit or line shape is unknown, use `stats_only`. If `fits_visible_context=true` and the task needs the whole content, use one whole-path read. If fit is false or unknown, choose the task-relevant route among `react.rg` to locate relevant regions in searchable text, bounded line/symbol reads, and programmatic inspection that writes small derived artifacts. If a whole-path read is truncated, continue through the task-relevant route. Tool outputs share the same visible-context caps. If an image exceeds the visible-context byte cap, use `react.read` on its `conv:fi:` path; when possible, `react.read` returns a bounded downscaled `image_view`. If a PDF or unsupported binary exceeds the cap, use a file-processing capability to extract or split it into smaller derived artifacts, then inspect those artifacts. Scope conclusions to the evidence you inspected and name the recovery method you used.
...
- Artifact URIs remain durable across turns. If external or historical content is needed beyond its visible representation, call `react.pull` for a readonly reference or `react.checkout` for editable current-turn state.
...
- Tool intents: `react.read` = visible context is enough; `react.pull` = code/tools need readonly local bytes; `react.checkout(items=[{"from":<ref>,"to":<target>,"strategy":"replace"|"overlay"}])` = resolve a `conv:fi:` or owner ref directly into editable current-turn `git/projects/...` or `files/...` state (`replace` resets file/dir; `overlay` is directory-only).
...
- Continue a previous workspace: checkout its `conv:fi:...git/projects/<scope>` ref directly to `git/projects/<scope>` with `replace`, then edit `turn_<current>/git/projects/<scope>/...`.
...
- Read ANNOUNCE `[WORKSPACE]` first when workspace state matters: `current editable workspace` = already-local editable tree; `previous saved workspace paths` = top-level `git/projects/...` paths from earlier turns — pull for readonly comparison, checkout directly to edit.
"""
...
- Ordinary document rendering and user-facing prose go through the renderer tools and `react.write`; renderer failed → fix renderer content/layout and retry the renderer. Do not reroute a document task through another tool when a renderer contract fits.
"""
