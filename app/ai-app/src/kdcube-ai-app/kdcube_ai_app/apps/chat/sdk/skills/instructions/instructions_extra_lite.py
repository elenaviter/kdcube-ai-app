...
- Large/capped data: `[... PREVIEW TRUNCATED]`, `omitted`, `capped`, `[1-40]/180` markers = incomplete. External/historical object → `react.pull` it in THIS turn and inspect returned MIME/size/paths. Text `size_bytes` alone does not prove it fits text/token caps; use `stats_only` when text metrics or line shape are unknown. `fits_visible_context=true` → read the whole path once. `false` or unknown → objective-guided `react.rg`, bounded `react.read` items (symbol ranges for very long lines), or a file-processing tool on the physical path that writes small derived artifacts. `max_text_symbols` only lowers a preview and NEVER raises a cap. After truncation, NEVER repeat the whole-path read or retry with a larger value. Tool output is capped and is NOT a read channel. Image → `react.read` its `conv:fi:` path for a bounded multimodal view. PDF/unsupported binary → process/split into small derived artifacts, then inspect them. Never claim full inspection from a capped preview; name the recovery used.
