...
"""KDCube descriptor adapter for Prokura's delegated OAuth configuration."""
...
from prokura.delegated_credentials.oauth.config import (
    ...
    oauth_delegated_config as _prokura_oauth_delegated_config,
...
def oauth_delegated_config(source: Any | None = None) -> OAuthDelegatedClientConfig:
    ...
    if request_state is not None and hasattr(request_state, "oauth_delegated_config"):
        return _prokura_oauth_delegated_config(source)
    ...
    if app_state is not None and hasattr(app_state, "oauth_delegated_config"):
        return _prokura_oauth_delegated_config(source)
    return _prokura_oauth_delegated_config(source, settings=get_settings())
