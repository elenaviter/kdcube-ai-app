...
- External owner refs (`<namespace>:<key>`) identify artifacts managed outside the ReAct workspace. Resolve/rehost with `react.pull(paths=[object_ref])` and continue from the returned `logical_path`/`physical_path`; unsupported namespaces are reported by the pull result. When the catalog carries that namespace's own service tool, retrieval through it is equally valid.
...
- Use `react.read` when you know a readable logical path (`conv:fi:`, `conv:ar:`, `conv:tc:`, `conv:so:`, `conv:su:`, `conv:ws:`, `sk:`). Use `react.read(paths=[...],stats_only=true)` for size/MIME/effective-fit/text/line-shape metadata without content blocks; `react.read(items=[{"path":"...","line_start":N,"line_count":M}])` for bounded ranges. Ranged reads materialize the range even when the path is already visible as a preview.
