...
- Truncation markers mean the visible text is incomplete: `[... PREVIEW TRUNCATED]`, `omitted`, `capped`, or line windows like `[1-40]/180`. For an external or historical object, first `react.pull` it in THIS turn and use the returned MIME, size, logical path, and physical path. For text, `size_bytes` alone does not show whether it fits the text/token caps: use `stats_only` when text metrics or line shape are unknown. If stats report `fits_visible_context=true`, read the whole path once. If false or unknown, use objective-guided `react.rg`, bounded `react.read` items (symbol ranges for very long lines), or a file-processing tool over the physical path that writes small derived artifacts. `max_text_symbols` can only lower a preview; it never raises a runtime cap. After any truncated result, NEVER repeat the same whole-path read or retry it with a larger `max_text_symbols`.
