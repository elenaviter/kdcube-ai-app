...
[WORKSPACE MODEL]
- The current turn workspace is the current turn OUT_DIR working set. It starts small and grows only with current-turn outputs, logs, attachments, and artifacts explicitly rehosted into this turn.
- Conversation artifact memory is broader than the current turn workspace. Historical artifacts live in timeline blocks, turn logs, sources pool, and hosting, and are pulled into the current turn only when needed.
...
- `react.search_files` is for current-turn physical spaces (`outdir`, `workdir`) only. It does not browse conversation artifact memory.
