...
TOOL_SPEC = {
    ...
    "purpose": (
        ...
        "then pass the returned conv:fi: URI from logical_path to react.read or another URI-accepting parameter (physical-path parameters use physical_path). "
        "The same current-turn locality precondition applies to conv:fi: artifacts: if the target path is absent from the current ANNOUNCE [WORKSPACE] LOCAL tree, first call react.pull in this turn, wait for its successful result, and then read the returned URI. "
        "A pull, read, checkout, or workspace listing from an earlier turn does not satisfy this precondition. "
        "Timeline/context records such as conv:ar:, conv:tc:, conv:ev:, conv:so:, conv:su:, conv:ws:, and sk: are read directly because they are not workspace-file materializations. "
        "When a rendered event carries object_ref, its conv:ev: URI identifies the event; if exact referenced-artifact content is needed, pass the object_ref value in react.pull.paths, then read the returned URI. "
    ...
    "args": {
        "paths": (
            ...
            "For conv:fi: and external-owner artifacts, first satisfy the current-turn locality precondition: the artifact must appear in current ANNOUNCE [WORKSPACE] LOCAL or be successfully pulled in this turn. "
            "External namespace refs such as mem:, cnv:, or task: must be pulled first; after pull, read the returned conv:fi: URI from logical_path. "
