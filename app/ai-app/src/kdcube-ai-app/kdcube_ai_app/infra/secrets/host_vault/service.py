...
3. replay: ``issued_at`` within the skew window; a mutating request id seen
   before answers the recorded result when its body digest matches, and
...
from kdcube_ai_app.infra.secrets.host_vault.audit import AuditSink, MemoryAuditSink, event_now
...
class HostVaultService:
    ...
    def handle(self, body: Any, *, peer_cert_pem: bytes | None, now: float | None = None) -> VaultResponse:
        ...
        except BaseException as exc:  # noqa: BLE001 - every failure becomes a fixed code
    ...
    def _dispatch(self, request: VaultRequest, identity: TrustRecord, moment: float) -> VaultResponse:
        ...
        if request.operation is Operation.HEALTH:
            return VaultResponse.success(request, protocol=PROTOCOL_VERSION, deployment_id=identity.deployment_id)
    ...
    def _mutate(self, request: VaultRequest, identity: TrustRecord) -> VaultResponse:
        ...
        if request.operation is Operation.DELETE:
            record = self._store.delete(reference, expected_generation=request.expected_generation)
        else:
            if request.operation is Operation.ROTATE and request.expected_generation is None:
                # A rotation supersedes an EXISTING value; without a stated
                # generation it must at least find one.
                if self._store.get(reference) is None:
                    raise VaultError(ErrorCode.NOT_FOUND)
            record = self._store.put(
                reference, (request.value or "").encode("utf-8"),
                expected_generation=request.expected_generation,
            )
        response = VaultResponse.success(request, generation=record.generation)
        with self._lock:
            self._seen[key] = _Seen(body_digest=digest, response=response, at=time.time())
    ...
    def _record(self, identity: TrustRecord | None, request: VaultRequest | None, request_id: str,
        ...
        except Exception:  # noqa: BLE001 - audit failure must not leak into the response
            LOGGER.warning("[host-vault] audit append failed", exc_info=True)


__all__ = ["HostVaultService", "REPLAY_CACHE_SIZE"]
