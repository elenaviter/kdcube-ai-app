...
---
# Instruction Blocks And Signals

Instruction blocks are the composition units of an agent's instruction body:
named, signal-carrying teachings (`REACT_LITE_*` moderate,
`REACT_XLITE_*` extra-lite, in
`kdcube_ai_app/apps/chat/sdk/skills/instructions/`) composed by an
agent-neutral composer. Any harness that builds an instruction body from
blocks uses this vocabulary; the ReAct decision agent is one such consumer
(its composition and caching story:
[the React system-instruction doc](../agents/react/system-instruction-README.md)).
This page owns the vocabulary, the signal table for every default block, and
the completeness checklist for lite/custom bodies.
...
| Harness context and artifact access | Everything in rendered context is an artifact and every artifact has a URI; artifacts can be represented anywhere in context; turns have fresh worker-local artifact workspaces; `conv:fi:` and external-owner artifacts must be present in current ANNOUNCE LOCAL or pulled in the current turn before `react.read` or file processing; timeline/context records remain directly readable; reported shape selects whole or bounded/programmatic inspection. | `REACT_HARNESS_CONTEXT_AND_ARTIFACT_ACCESS`, inserted by `compose_decision_system_text(...)` after the strict protocol. | Not a selectable Lite/Extra Lite block. Always present for Full, Lite, Extra Lite, and explicit complete bodies. |
...
| `react.memsearch` recovery | Find prior conversation material when the exact path is unknown, with the query shaped to the landscape it searches: content words as they would appear in the stored text, exact strings quoted, `OR` between synonyms, time words moved to `from`/`to`, one topic per call. Why: the lexical arm ANDs every unquoted word and the fuzzy arm averages over the query tokens, so conversational framing ("last time I worked with...") finds nothing. | `MEMORY_RECOVERY_GUIDE`; the full mechanics ride the `react.memsearch` tool spec in the catalog (`CONVERSATION_QUERY_GUIDE` from `sdk/solutions/conversation/instructions.py`), rendered in full under both catalog detail modes. | `REACT_LITE_MEMORY_SEARCH_RECOVERY` (short form with the same rule); `workspace`, `workspace_exec`, `document`, `web`, `all_capabilities`. |
| Turn summary written for retrieval | The `channel:summary` text is the turn's memory stone for the model's future self: named by searchable names (the user's wording, file names, identifiers), distinguishable from the earlier turns visible above it (what is NEW in this turn), with Retrieval-anchors (verbatim phrases, turn-identifying proper nouns) for the lexical arm. Why: a summary a query cannot tell apart from its neighbours is unfindable however good the search is; writing and querying are one mechanism. | Part of the strict channel protocol: `react_summary_channel_details(...)` in `agents/decision.py` (v2, v3), rendering `turn_summary_writing_guide` from `sdk/solutions/conversation/instructions.py`; names `react.memsearch` only when that tool is bound. | Not a lite block. Always present in the protocol for every profile. Hosted agents receive the same guide through `turn_summary_contribution_guide` (`[SHARED TURN CONTEXT]`). |
