...
| `named_services.schema_search.vector_backend` | `faiss-local` | `BaseEntrypoint`, named-service capability search | Vector backend for projected provider capability catalogs: persistent `faiss-local`, or explicit dependency-free `bruteforce` fallback |
...
| `named_services.schema_search.vector_backend` | `self.bundle_prop("named_services.schema_search.vector_backend")` | `<prefix>/bundles/<bundle_id>/descriptor` `config.named_services.schema_search.vector_backend` | `bundles.yaml -> items[].config.named_services.schema_search.vector_backend` | cache | `faiss-local` persists the derived vector view beside the capability SQLite file; `bruteforce` reconstructs an in-memory view per process |
