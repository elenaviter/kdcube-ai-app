...
def test_primary_agents_are_visible_at_the_agents_root() -> None:
    ...
    assert set(ADAPTERS).issubset(top_level_directories)
    assert {"shared", "advanced"}.issubset(top_level_directories)
    assert "demos" not in top_level_directories
    assert "quickstart" not in top_level_directories
...
def test_examples_share_one_standard_app_agnostic_descriptor_set() -> None:
    descriptors = SHARED_ROOT / "descriptors.template"
    ...
    assert not [
        ...
        if (AGENTS_ROOT / adapter / "descriptors.template").exists()
    ]
...
def test_claude_uses_sdk_git_session_store_for_its_transcript() -> None:
    ...
    assembly = yaml.safe_load(
        (SHARED_ROOT / "descriptors.template" / "assembly.yaml").read_text(
            encoding="utf-8"
        )
    )
    secrets = yaml.safe_load(
        (SHARED_ROOT / "descriptors.template" / "secrets.yaml").read_text(
            encoding="utf-8"
        )
    )
...
def test_native_model_selection_comes_from_assembly_descriptor(tmp_path: Path) -> None:
    ...
    shutil.copytree(SHARED_ROOT / "descriptors.template", descriptors)
...
def test_shared_research_skill_is_real_and_selected_by_every_example() -> None:
    skill = SHARED_ROOT / "skills" / "demo" / "research-brief" / "SKILL.md"
    ...
    for adapter in ADAPTERS:
        ...
        assert config["agent"]["skills"]["root"] == "../shared/skills"
...
def test_missing_isolated_exec_image_fails_before_agent_construction(
    ...
    from agents.shared import configuration
...
def test_independent_infrastructure_compose_contract() -> None:
    compose = yaml.safe_load((SHARED_ROOT / "compose.yaml").read_text(encoding="utf-8"))
...
def test_configure_materializes_standard_descriptors_without_printing_secrets(
    ...
    from agents.shared.configure import configure

    shutil.copytree(
        SHARED_ROOT / "descriptors.template", tmp_path / "descriptors.template"
    )
...
def test_infrastructure_projects_standard_descriptor_settings(tmp_path: Path) -> None:
    from agents.shared.infrastructure import postgres_url, redis_url, storage_uri
